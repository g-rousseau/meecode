...
trajectory.position_reference(:,1) = [0; y_ref_mag * sin(y_ref_phase)];
trajectory.k = 1;

display_config = display_init(init_state, trajectory);
display_config = display_state(time, state, trajectory, display_config);

while time <= simulation_duration
    % reference
    position_ref = [x_ref_mag * sin(x_ref_frequency * time); ...
                    y_ref_mag * sin(y_ref_frequency * time + y_ref_phase)];
    velocity_ref = [x_ref_mag * x_ref_frequency * cos(x_ref_frequency * time); ...
                    y_ref_mag * y_ref_frequency * cos(y_ref_frequency * time + y_ref_phase)];
                
    % position control signal
    position_gain = 1;
    reference_velocity = position_gain * (position_ref - state.position) + velocity_ref;
%     reference_velocity = position_gain * (position_ref - state.position);
...
function display_config = display_init(init_state, trajectory)
